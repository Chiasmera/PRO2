...
public class Controller {
    ...
    public PN opretPNOrdination(LocalDate startDen, LocalDate slutDen, Patient patient, Laegemiddel laegemiddel, double antal) {
        ...
        if (checkStartFoerSlut(startDen, slutDen)) {
            throw new IllegalArgumentException("Startdatoen er efter slutdatoen");
        } else {
            pn = new PN(startDen, slutDen, antal, patient);
    ...
    public DagligFast opretDagligFastOrdination(LocalDate startDen, LocalDate slutDen, Patient patient, Laegemiddel laegemiddel, double morgenAntal, double middagAntal, double aftenAntal, double natAntal) {
        ...
        if (startDen.isAfter(slutDen)) {
            throw new IllegalArgumentException("Startdatoen må ikke ligge efter slutdato");
        } else {
            dagligFast = new DagligFast(startDen, slutDen, patient);
            dagligFast.setLaegemiddel(laegemiddel);
            dagligFast.opretDosis(LocalTime.of(9, 0), morgenAntal);
            dagligFast.opretDosis(LocalTime.of(15, 0), middagAntal);
            dagligFast.opretDosis(LocalTime.of(21, 0), aftenAntal);
            dagligFast.opretDosis(LocalTime.of(3, 0), natAntal);
        }
        return dagligFast;
    }
    ...
    public DagligSkaev opretDagligSkaevOrdination(LocalDate startDen, LocalDate slutDen, Patient patient, Laegemiddel laegemiddel, LocalTime[] klokkeSlet, double[] antalEnheder) {
        ...
        if (checkStartFoerSlut(startDen, slutDen)) {
            ...
            throw new IllegalArgumentException("Længde på klokkeslet og antal enheder skal match");
        }
    ...
    public void ordinationPNAnvendt(PN ordination, LocalDate dato) {
        if (!ordination.givDosis(dato)) {
            throw new IllegalArgumentException("Angivne dato ligger ikke indenfor ordinationens gyldighedsperiode");
        }
    }
    ...
    public double anbefaletDosisPrDoegn(Patient patient, Laegemiddel laegemiddel) {
        double enhedPrKg = 0.0;

        if (patient.getVaegt() < 25) {
            enhedPrKg = laegemiddel.getEnhedPrKgPrDoegnLet();
        } else if (patient.getVaegt() > 120) {
            enhedPrKg = laegemiddel.getEnhedPrKgPrDoegnTung();
        } else {
            enhedPrKg = laegemiddel.getEnhedPrKgPrDoegnNormal();
        }

        return patient.getVaegt() * enhedPrKg;
    }
     ...
     *
     * @param vægtStart   Starten af vægtintervallet
     * @param vægtSlut    slutningen af vægtintervallet
     * @param laegemiddel Et lægemiddel
     * @return antallet af ordinationer af det angivne lægemiddel, med patienter indenfor det angivne vægtinterval
     */
