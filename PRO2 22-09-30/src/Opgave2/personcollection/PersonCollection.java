...
public class PersonCollection implements Iterable<Person> {
    ...
    private class PersonIterator implements Iterator<Person> {
        ...
        private int sidstKendteSize = size;

        @Override
        public boolean hasNext() {
            if (persons[position] == null || position >= size) {
               return false;
            } else {
                return true;
            }
        ...
        @Override
        public Person next() {
            if (sidstKendteSize == size) {
                Person passedOver = persons[position];
                position++;
                return passedOver;
            } else {
                throw new ConcurrentModificationException("Samlingen er blevet ændret siden iteratoren blev skabt");
            }
